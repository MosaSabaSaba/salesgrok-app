...
// Wire up the button
function initClearResultsBtn() {
  const btn = document.getElementById('resultsClearBtn');
  if (!btn) return;
  btn.addEventListener('click', () => {
    // Hide the whole results area
    const resultsArea = document.getElementById('resultsArea');
    resultsArea.classList.remove('visible');

    // Reset internal result state
    lastGeneratedOriginal = [];
    lastGeneratedStyled = [];

    // Clear the DOM inside both result containers
    const single = document.getElementById('resultItemsSingle');
    const original = document.getElementById('resultItemsOriginal');
    const styled = document.getElementById('resultItemsStyled');
    if (single) single.innerHTML = '';
    if (original) original.innerHTML = '';
    if (styled) styled.innerHTML = '';

    // Deactivate the active history highlight
    activePromptIndex = null;
    renderHistory(document.getElementById('searchInput')?.value || '');

    showToast('Results cleared', 'fa-xmark');
  });
}
